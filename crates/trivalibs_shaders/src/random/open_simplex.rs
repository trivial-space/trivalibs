...
// Replace std imports with core for no_std compatibility
...
pub fn noise2(seed: i64, x: f64, y: f64) -> f32 {
	...
	noise2_unskewed_base(seed, xs, ys)
}
...
*/
pub fn noise2_improve_x(seed: i64, x: f64, y: f64) -> f32 {
	// Skew transform and rotation baked into one.
	let xx = x * ROOT2OVER2;
	let yy = y * (ROOT2OVER2 * (1.0 + 2.0 * SKEW_2D));

	noise2_unskewed_base(seed, yy + xx, yy - xx)
}

/**
	2D Simplex noise base.
*/
fn noise2_unskewed_base(seed: i64, xs: f64, ys: f64) -> f32 {
...
*/
pub fn noise3_improve_xy(seed: i64, x: f64, y: f64, z: f64) -> f32 {
	...
	noise3_unrotated_base(seed, xr, yr, zr)
}
...
*/
pub fn noise3_improve_xz(seed: i64, x: f64, y: f64, z: f64) -> f32 {
...
*/
fn noise3_unrotated_base(seed: i64, xr: f64, yr: f64, zr: f64) -> f32 {
		...
		if ax0 >= ay0 && ax0 >= az0 {
			...
			if b > 1.0 {
				...
				value += (b * b)
					* (b * b) * grad3(
					seed,
					xrbp - Wrapping(x_nsign as i64) * Wrapping(PRIME_X),
					yrbp,
					zrbp,
					xri + x_nsign as f32,
					yri,
			...
			if b > 1.0 {
				...
				value += (b * b)
					* (b * b) * grad3(
					seed,
					xrbp,
					yrbp - Wrapping(y_nsign as i64) * Wrapping(PRIME_Y),
					zrbp,
					xri,
					yri + y_nsign as f32,
					zri,
			...
			if b > 1.0 {
				...
				value += (b * b)
					* (b * b) * grad3(
					...
					xrbp,
					yrbp,
					zrbp - Wrapping(z_nsign as i64) * Wrapping(PRIME_Z),
					xri,
					yri,
					zri + z_nsign as f32,
				);
...
*/
pub fn noise4_improve_xyz_improve_xy(seed: i64, x: f64, y: f64, z: f64, w: f64) -> f32 {
	let xy = x + y;
	...
	noise4_unskewed_base(seed, xr, yr, zr, wr)
}
...
*/
pub fn noise4_improve_xyz_improve_xz(seed: i64, x: f64, y: f64, z: f64, w: f64) -> f32 {
	let xz = x + z;
...
*/
pub fn noise4_improve_xyz(seed: i64, x: f64, y: f64, z: f64, w: f64) -> f32 {
	let xyz = x + y + z;
	let ww = w * 0.2236067977499788;
	...
	noise4_unskewed_base(seed, xs, ys, zs, ws)
}
...
*/
pub fn noise4_improve_xy_improve_zw(seed: i64, x: f64, y: f64, z: f64, w: f64) -> f32 {
...
*/
fn noise4_unskewed_base(seed: i64, xs: f64, ys: f64, zs: f64, ws: f64) -> f32 {
	let mut seed = Wrapping(seed);
...
fn grad4(
	...
	(GRADIENTS_4D[gi | 0] * dx + GRADIENTS_4D[gi | 1] * dy)
...
// Pre-computed gradient arrays using const initialization
const GRADIENTS_2D: [f32; N_GRADS_2D as usize * 2] = generate_gradients_2d();
const GRADIENTS_3D: [f32; N_GRADS_3D as usize * 4] = generate_gradients_3d();
const GRADIENTS_4D: [f32; N_GRADS_4D as usize * 4] = generate_gradients_4d();

// Generate 2D gradients at compile time
const fn generate_gradients_2d() -> [f32; N_GRADS_2D as usize * 2] {
	let mut result = [0.0; N_GRADS_2D as usize * 2];
	let mut i = 0;
	while i < N_GRADS_2D as usize * 2 {
		let src_idx = i % (GRAD2_SRC.len());
		result[i] = (GRAD2_SRC[src_idx] / NORMALIZER_2D) as f32;
		i += 1;
	}
	result
}

// Generate 3D gradients at compile time
const fn generate_gradients_3d() -> [f32; N_GRADS_3D as usize * 4] {
	let mut result = [0.0; N_GRADS_3D as usize * 4];
	let mut i = 0;
	while i < N_GRADS_3D as usize * 4 {
		let src_idx = i % (GRAD3_SRC.len());
		result[i] = (GRAD3_SRC[src_idx] / NORMALIZER_3D) as f32;
		i += 1;
	}
	result
}

// Generate 4D gradients at compile time
const fn generate_gradients_4d() -> [f32; N_GRADS_4D as usize * 4] {
	let mut result = [0.0; N_GRADS_4D as usize * 4];
	let mut i = 0;
	while i < N_GRADS_4D as usize * 4 {
		let src_idx = i % (GRAD4_SRC.len());
		result[i] = (GRAD4_SRC[src_idx] / NORMALIZER_4D) as f32;
		i += 1;
	}
	result
}
